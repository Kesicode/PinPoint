...
export interface GnssMeasurementsStream {
  ...
  error: string | null;
  status: PinPointGnssStatus | null;
  supported: boolean | null;
}

/**
 * Streams raw GNSS C/N0 measurement epochs from the PinPointGnss native module
...
export function useGnssMeasurements(historyLength = 600): GnssMeasurementsStream {
  ...
  const [status, setStatus] = useState<PinPointGnssStatus | null>(null);
  ...
  useEffect(() => {
    ...
    const measurementSubscription = PinPointGnss.addListener('onMeasurement', (event) => {
      if (cancelled) return;
    ...
    const errorSubscription = PinPointGnss.addListener('onError', (event) => {
      if (cancelled) return;
      setError(`${event.code}: ${event.message}`);
    });

    const statusSubscription = PinPointGnss.addListener('onStatus', (event) => {
      if (cancelled) return;
      setStatus(event.status);
    });

    (async () => {
      const isSupported = PinPointGnss.isSupported();
      if (cancelled) return;
