/**
 * PinPoint SDK shared contract.
 *
 * These types are the integration contract between PinPoint-sdk and consumers
 * (apps/PinPoint-demo). They must not change shape without a coordinated update
 * on both sides.
 */
...
/** One OS-level network-integrity sample (real PinPointNet probe output). */
...
export interface SensorWindow {
  ...
  /**
   * OS-level network-integrity signal for the current instant (PinPointNet).
