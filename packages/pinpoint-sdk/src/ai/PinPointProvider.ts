...
export interface PinPointProviderProps extends PreloadOptions {
 ...
 * module-level caches that `createPinPointSDK()` reads, so a mounted provider
...
export function PinPointProvider({
  ...
  textEmbeddings = true,
  children,
}: PinPointProviderProps): ReactNode {
  useEffect(() => {
