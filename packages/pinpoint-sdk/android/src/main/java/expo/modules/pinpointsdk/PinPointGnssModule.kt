...
private const val TAG = "PinPointGnss"
...
/**
 * PinPointGnss — raw GNSS C/N0 measurement streaming for the PinPoint integrity pipeline.
 *
...
class PinPointGnssModule : Module() {
    ...
    private fun startStreaming(promise: Promise) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.N) {
            val message = "PinPointGnss requires Android 7.0 (API 24) or newer for GnssMeasurements; this device is API ${Build.VERSION.SDK_INT}."
        ...
        if (!hasFineLocationPermission()) {
            val message = "ACCESS_FINE_LOCATION has not been granted. Request location permission in the app before calling PinPointGnss.start()."
        ...
        try {
            ...
            measurementCallback = null
            val message = "ACCESS_FINE_LOCATION is required for raw GNSS measurements and was not granted (SecurityException). Request location permission in the app before calling PinPointGnss.start()."
            Log.e(TAG, message, e)
            emitError("E_PERMISSION", message)
            emitStatus("notAllowed")
