...
PinPoint watches your phone's raw GNSS signals and sensor streams, runs six physics
...
bare workflow build** — the native `PinPointGnss` module is compiled by the EAS
...
```tsx
import { PinPointProvider, createPinPointSDK, useLocationStream, useImuStream, useBarometerStream, useGnssMeasurements } from 'pinpoint-sdk';
...
<PinPointProvider>{/* your app */}</PinPointProvider>
...
const verdict = PinPoint.evaluate({ fixes, imu: imuSamples, baro: baroSamples, gnss });

console.log(verdict.state, verdict.reason, verdict.confidence);

// 4. Explain the verdict on demand (never mutates state):
const sentence = await PinPoint.explain(verdict);
```
...
`createPinPointSDK().evaluate()` owns the debounce counter internally; the
...
- **No model loads at startup.** Models load lazily on first use; `PinPointProvider`
  pre-warms them in the background (same shared instances).
- **AI never touches state.** `PinPointSDK` exposes no mutation path —
...
### `createPinPointSDK(): PinPointSDK`

```ts
interface PinPointSDK {
...
`transcribeCommand`, `embedText`, `RingBuffer`, and the `PinPointGnss` binding.
...
The native `PinPointGnss` module compiles on EAS; measurement streaming itself
