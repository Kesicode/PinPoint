...
**On-device GPS integrity monitoring for Android.** PinPoint continuously cross-examines raw GNSS measurements against the phone's own physics — inertial sensors, barometer, magnetometer, and the sun itself — through six independent consistency checks, then feeds them into a deterministic RAIM/FDE-derived state machine that classifies every fix as **TRUSTED**, **DEGRADED**, **DENIED**, or **RECOVERING**. A fully offline AI stack — an on-device LLM, speech recognizer, and embedder running via ExecuTorch — explains every verdict, takes voice commands, and powers semantic search over the flight-recorder log, without ever sending a byte to the cloud.

The repo is an npm-workspaces monorepo:

- [`packages/pinpoint-sdk`](packages/pinpoint-sdk) — the engine: Expo native module (Android) exposing raw satellite measurements, sensor streams, six physics checks, the RAIM/FDE state machine, and the on-device AI layer.
- [`apps/pinpoint-demo`](apps/pinpoint-demo) — the cockpit: an Expo SDK 57 demo app with an avionics glass-cockpit UI that exercises every SDK capability.
...
- [PinPointSDK interface](#PinPointSDK-interface)
- [Roadmap](#roadmap)
- [License](#license)
...
┌──────────────────────────────────────────────────────────────────────┐
│ 1 · SENSORS                                                          │
│ GNSS measurements (Kotlin PinPointGnss, API 24+) · location @ 1 Hz     │
...
┌──────────────────────────────────────────────────────────────────────┐
│ 7 · CONSUMER                                                         │
│ PinPointProvider → Verdict { state, checks[], reason }                 │
...
| Path | Role |
| --- | --- |
| `packages/pinpoint-sdk` | Expo native module (standalone, **Android-only**): Kotlin GNSS bridge, sensor hooks, six checks, state machine, AI loaders. Pure logic is platform-agnostic TypeScript. |
| `apps/pinpoint-demo` | Expo SDK 57 app (expo-router) with the avionics UI, voice control, semantic search, and debug controls. Consumes the SDK via the workspace. |

Root `package.json` wires both with npm workspaces; `npm install` at the root links `pinpoint-sdk` into the demo app.
...
3. The resulting `Verdict` (state, per-check results, human-readable reason) is published through `PinPointProvider`.
...
The demo (`apps/pinpoint-demo`, Expo SDK 57 + expo-router, distributed as an EAS-built dev client) presents the pipeline as an **avionics glass cockpit**:
...
Before any native dialog appears, a **permissions primer screen** explains, in plain language, why PinPoint wants location (it is the thing being monitored) and the microphone (voice commands only). Tapping **Continue** triggers the native dialogs **in sequence**, so each request arrives with context instead of a permission blitz on first launch.
...
```bash
git clone https://github.com/Kesicode/PinPoint.git
cd PinPoint
npm install
```
...
Launch the installed PinPoint dev client; it connects to the Metro bundler.

### Test and typecheck

```bash
cd packages/pinpoint-sdk
...
├── packages/
│   └── pinpoint-sdk/               # Expo module — standalone, Android-only
│       ├── package.json          # no-build TS package: main/types → src/index.ts
│       ├── expo-module.config.json
│       ├── src/
│       │   ├── index.ts          # public barrel: createPinPointSDK, hooks, provider, types
│       │   ├── PinPointSDK.types.ts
...
## PinPointSDK interface

```ts
import { createPinPointSDK } from "pinpoint-sdk";

const PinPoint = createPinPointSDK();

// Safety path — pure, synchronous, deterministic.
const verdict: Verdict = PinPoint.evaluate(sensorWindow, previousState);

// Advisory path — async, lazy-loaded, fully on-device (ExecuTorch / XNNPACK).
const explanation = await PinPoint.explain(verdict);   // "Fix denied: position jumped 1.2 km in one second."
const transcript  = await PinPoint.transcribe(pcmF32); // Float32Array, 16 kHz mono → "simulate spoof"
const vector      = await PinPoint.embed("signal looked synthetic"); // number[] for cosine search
```
...
- `createPinPointSDK(): PinPointSDK` — factory wiring native module, checks, state machine, and AI loaders
- **Sensor hooks:** `useLocationStream` (expo-location @ 1 Hz) · `useImuStream` (magnetometer + gyroscope complementary heading) · `useBarometerStream` · `useGnssMeasurements` (per-satellite C/N0 via the Kotlin `PinPointGnss` module, ring buffer)
- `PinPointProvider` — React context publishing the latest `Verdict`
