...
### SDK (packages/pinpoint-sdk)
- NEW 7th check `networkCheck` (`src/physics/networkCheck.ts`): a real OS VPN signal (`SensorWindow.network`, from PinPointNet) fails the check while a tunnel is up — the instrument never holds TRUSTED with a VPN; absent signal abstains (passes with note), never invents values
...
- NETWORK panel: real PinPointNet VPN row (TUNNEL ACTIVE — INCONSISTENT / no tunnel), check FAIL flag, IP↔GPS divergence, resolved IP geo (ipwho.is)
...
- VPN end-to-end with a real VpnService (TinyVpn test fixture, `appops ACTIVATE_VPN allow`): tunnel up → PinPointNet TUNNEL ACTIVE + network check FAIL → **DEGRADED "network failed"** (physics still 100) → tunnel down → debounced recovery → TRUSTED; flight log records every detect/clear with NETWORK labels
...
## 2026-08-29 — feat: scaffold packages/pinpoint-sdk (expo module, android)
...
- full rewrite: pitch, TOC, background (RAIM/FDE heritage, on-device AI rationale), 7-stage ASCII pipeline, features (six checks w/ spoofer rationale, solar compass, state machine table, AI stack, voice, semantic search, demo UI, permissions), design tokens, getting started, project tree, PinPointSDK reference, roadmap, license

## 2026-08-29 — feat: sensor hooks + shared contract types
- src/types.ts: exact PinPointSDK contract types (IntegrityState, CheckId, CheckResult, Fix, ImuSample, BaroSample, SatelliteMeasurement, GnssMeasurementSample, SensorWindow, Verdict)
- useLocationStream (1 Hz Balanced, no auto permission request), useImuStream (mag+gyro ~10 Hz, atan2 portrait heading + complementary filter), useBarometerStream (~10 Hz), useGnssMeasurements (PinPointGnss native stream, ring-buffered history)
- src/utils/ringBuffer.ts shared FIFO util

## 2026-08-29 — feat: pinpoint-demo scaffold + app config + design system + permissions primer (apps/)
- create-expo-app default template (SDK 57, expo-router TS); installed expo-dev-client/location/sensors/audio/haptics/font/image, reanimated, async-storage, Google Fonts Inter + IBM Plex Mono; pinpoint-sdk workspace link (npm hoists to root node_modules)
- app.json: name PinPoint, slug PinPoint, scheme PinPoint, dark UI, #0C1116, edge-to-edge, android package com.kesicode.PinPoint, location+mic permissions, expo-audio mic plugin copy, expo-font plugin
- eas.json: cli.version >= 16.0.1, cli.appVersionSource remote (eas-cli 23 moved the key), development (dev-client, internal, autoIncrement) + production profiles; eas init @kesicode/PinPoint (0dc6739c-4d27-4709-9693-13d29dba3de2); early EAS android development build queued: ffd2cd1e-c829-49d1-ba9a-aca0e681129b
  - note: first build attempt failed generating cloud keystore (truncated request error); immediate retry succeeded — transient
- scripts/generate-assets.mjs (sharp): 1024px avionics PinPoint-in-crosshair glyph (#00D9A3 on #0C1116, hairline #3A434D grid) -> icon, splash, adaptive foreground/background/monochrome, favicon; default Expo template assets deleted
...
- src/app/_layout.tsx: loads both font families, wraps Stack in SDK PinPointProvider (pending export in pinpoint-sdk — tracked), dark content style
...
## 2026-08-29 — feat: ExecuTorch AI wrappers (explain/transcribe/embed), PinPointProvider, createPinPointSDK
...
- PinPointProvider: headless preloader sharing the SDK's model caches; createPinPointSDK: owns the IntegrityMachine for debounced recovery

## 2026-08-29 — feat: full SDK wiring + package README
- src/index.ts exports the complete surface: native binding, sensor hooks, six checks, solarCompassHeading, evaluateIntegrity/stepIntegrity, createPinPointSDK/PinPointProvider, AI wrappers, contract types
...
## 2026-08-29 — feat(pinpoint-demo): complete instrument app + first green EAS build (apps/)
- full instrument UI: StatusStrip (state fill + crossfade), six PFD-style TapeGauges (scrolling tick tape, fixed center readout, eased via Reanimated), EventLog flight recorder, BottomBar with labeled TEST HARNESS (SIMULATE SPOOF injects 5 teleported fixes + 5 lockstep-waveform C/N0 epochs through the normal pipeline; RESET swaps in a fresh createPinPointSDK to clear the SDK-internal debounce machine; SHOW REASON reveals last explanation), mic capture via expo-audio AudioStream (16 kHz mono float32, fully offline) -> sdk.transcribe -> fixed command matching, semantic search (sdk.embed query -> cosine vs stored reason vectors)
- pipeline aligned with final SDK semantics: no prevState threading (SDK owns recovery-debounce), PinPointProvider at router root, useGnssMeasurements(30)
- expo-doctor 21/21; tsc --noEmit clean against complete pinpoint-sdk contract
...
- EAS development build 254e5629-9aa6-476f-ac31-b04c800e9de9 FINISHED (post pinpoint-sdk Kotlin constellation fix 5ba7532); APK downloaded to apps/pinpoint-demo/releases/PinPoint-dev.apk (gitignored: 258 MB exceeds GitHub 100 MB blob limit — artifact lives at https://expo.dev/accounts/kesicode/projects/PinPoint/builds/254e5629-9aa6-476f-ac31-b04c800e9de9)
- APK artifact apps/pinpoint-demo/releases/PinPoint-dev.apk now stored in git via LFS (258 MB, exceeds plain-blob limit): https://expo.dev/accounts/kesicode/projects/PinPoint/builds/254e5629-9aa6-476f-ac31-b04c800e9de9

## 2026-08-29 — ci: dev apk published as rolling dev-latest GitHub release; future artifacts ship via releases, LFS frozen
- release https://github.com/Kesicode/PinPoint/releases/tag/dev-latest — asset PinPoint-dev.apk (258,207,665 bytes, state: uploaded), body carries EAS build URL + id 254e5629 + date + API 24+ physical-device install note
...
- EAS production build 81f660f9-453d-4446-b246-b9a84a37b077 FINISHED: https://expo.dev/accounts/kesicode/projects/PinPoint/builds/81f660f9-453d-4446-b246-b9a84a37b077 — verified real app: assets/index.android.bundle (4,107,444 B) embedded, zero dev-launcher/expo-dev-client entries in the APK
- rolling release replaced: dev-latest deleted, tag `latest` recreated with the production APK (184,483,397 B): https://github.com/Kesicode/PinPoint/releases/tag/latest
- in-repo LFS artifact replaced: PinPoint-dev.apk (frozen 254e5629 dev client) removed, PinPoint.apk added as new LFS object — superseded LFS object deletion not exposed by the GitHub REST API (404); ages out via orphaned-object GC
...
- EAS production build 6004d5eb-92b0-42a5-9f9b-84a802bf66c9 FINISHED: https://expo.dev/accounts/kesicode/projects/PinPoint/builds/6004d5eb-92b0-42a5-9f9b-84a802bf66c9 — verified: assets/index.android.bundle (4,107,360 B) embedded, 0 dev-launcher/expo-dev-client entries
- rolling release `latest` deleted + recreated with the new APK (184,483,309 B): https://github.com/Kesicode/PinPoint/releases/tag/latest
- LFS object replaced (45eb611492 -> d9996625d3) — commit 5a384c1

## 2026-08-29 — fix(pinpoint-demo): blank-launch hardening — ErrorBoundary, startup milestones, resilient font gate, react compiler off
...
- [PinPoint:startup] milestone logs (fonts/provider/primer decisions/dashboard mount) — `adb logcat | grep PinPoint:startup` pinpoints a stall
...
- EAS production build c6d4852a-50da-422c-ae54-13239678b47c FINISHED: https://expo.dev/accounts/kesicode/projects/PinPoint/builds/c6d4852a-50da-422c-ae54-13239678b47c
- verified in the shipped binary: assets/index.android.bundle embedded (4,103,332 B); extracted-bundle greps: 'PinPoint:startup' 1, 'qwen' 3 (Qwen3 accessor present), 'INTEGRITY FAULT' 1; 0 dev-launcher/expo-dev-client entries
- rolling release `latest` deleted + recreated with the new APK (184,480,273 B): https://github.com/Kesicode/PinPoint/releases/tag/latest
...
## 2026-08-29 — test(pinpoint-demo): component render suite — primer, font-gate resilience, error boundary, dashboard with real physics
...
- jest.config.js (jest-expo preset, jsdom, transformIgnorePatterns for expo/reanimated/pinpoint-sdk), jest.setup.ts -> native-module boundary mocks (expo-location/expo-audio/expo-haptics/AsyncStorage/PinPointGnss native binding/reanimated-worklets passthrough); physics/checks/state machine always real
...
- SDK: `PinPointProvider` now renders `children` (was `return null` — blank screen on device; tests mocked it); `evaluateIntegrity` off-by-one `reasonFor` now uses `next.cleanStreak` (DENIED 1/5 vs 0/5, RECOVERING satisfied after 5)
...
- SDK: new native `AnchorNetModule.kt` — real OS VPN detection (kernel tun/tap interface scan + ConnectivityManager TRANSPORT_VPN), registered in expo-module.config.json; `PinPointNet` exported from SDK index
- Demo `useNetworkIntegrity`: polls real `PinPointNet.isVpnActive()` every 2 s; real HTTPS IP geolocation (ipwho.is → ipapi.co fallback) every 60 s; real haversine IP↔GPS divergence (limit 150 km) — banner + flight-recorder events on tunnel up/down and divergence; GNSS physics stay authoritative (VPN ≠ spoof, per fraud-detection research)
