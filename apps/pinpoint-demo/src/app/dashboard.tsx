 ...
 *     NETWORK        — real PinPointNet VPN probe + IP↔GPS divergence panel
...
import { usePinPointPipeline } from '@/hooks/usePinPointPipeline';
import type { EventLogEntry } from '@/hooks/usePinPointPipeline';
...
import type { PinPointSDK, CheckId, CheckResult } from 'pinpoint-sdk';
...
export default function DashboardScreen() {
  ...
  const pipeline = usePinPointPipeline();
  ...
  const voice = useVoiceCommands(sdk as PinPointSDK, onCommand);
  ...
  useEffect(() => {
    ...
    if (prevNetRef.current.vpn !== null && net.vpnActive !== prevNetRef.current.vpn) {
      recordNetwork(
        net.vpnActive
          ? `VPN tunnel detected (PinPointNet) — IP ${net.ip ? `${net.ip.ip}${net.ip.city ? ` · ${net.ip.city}, ${net.ip.country ?? ''}` : ''}` : 'resolving'}; network integrity check FAILS while up`
