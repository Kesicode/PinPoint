/**
 * usePinPointPipeline — app-side wiring of the PinPoint-sdk integrity pipeline.
 *
 ...
 */
import { PinPointNet, useBarometerStream, useGnssMeasurements, useImuStream, useLocationStream } from 'pinpoint-sdk';
import type {
  PinPointSDK,
  BaroSample,
  CheckId,
...
export function usePinPointPipeline() {
  // The SDK instance owns the recovery-debounce machine; RESET replaces it.
  const [sdk, setSdk] = useState<PinPointSDK>(() => createPinPointSDK());
  ...
  // Real OS-level VPN signal (PinPointNet), sampled on a 2 s poll and read at
  ...
  // PinPointNet poll: sample the real OS VPN signal every 2 s into a ref so
  // each evaluation window carries the tunnel state at evaluation time.
  useEffect(() => {
    const poll = () => {
      try {
        vpnActiveRef.current = PinPointNet.isVpnActive();
      } catch {
  ...
  const evaluateNow = useCallback(() => {
    ...
    const { result: v, ms } = measureDeterministic(() =>
      sdk.evaluate({
        ...
        // Real OS VPN probe; omitted until the first PinPointNet poll lands so
