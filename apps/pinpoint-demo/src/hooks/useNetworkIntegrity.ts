 ...
 *
 *  - VPN tunnel detection via the PinPointNet native module: kernel tun/tap
...
import { PinPointNet, haversineMeters } from 'pinpoint-sdk';
...
export function useNetworkIntelligence(fix: { latitude: number; longitude: number } | null): NetworkIntegrity {
  ...
  useEffect(() => {
    const readVpn = () => {
      try {
        setVpnActive(PinPointNet.isVpnActive());
      } catch {
